...
function LoadingSkeleton({ data }: { data: SotNodeData }) {
  return (
    <div className="h-full rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 shadow-sm transition-colors duration-150 hover:border-gray-300 dark:hover:border-gray-600">
      ...
      <div className={`flex h-full flex-col rounded-lg border bg-white dark:bg-gray-800 shadow-sm transition-all duration-150 ${selected ? "border-blue-400 ring-2 ring-blue-400/30" : "border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"}`}>
        ...
        {isPdf && data.pdfUrl ? (
          ...
          <div className="nodrag min-h-0 flex-1 overflow-hidden cursor-text">
            <div className="px-4 pt-1 pb-0.5 text-[12px] font-semibold text-gray-300 dark:text-gray-600 truncate text-center">{data.title}</div>
            <RichTextEditor
          ...
          <>
            <div className={`${selected ? "nowheel" : ""} min-h-0 flex-1 overflow-y-auto px-4 pb-3 cursor-text`}>
              <div className="mx-auto max-w-xl text-xs leading-relaxed text-gray-600 dark:text-gray-300 prose prose-xs prose-gray dark:prose-invert">
